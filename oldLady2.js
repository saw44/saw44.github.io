...
while(x <=7){
  for (i = 0; i <= animal.length - 1; i++) {
    if (sent1.includes(animal[1])) {
      let newSent1 = sent1.replace(animal[1], animal[2]);
       sent3A = sent3.replace(animal[1], animal[2]);
       let newSent3A = sent3A.slice(0, 26) + ` catch the ` + animal[1] + `.\n`;
       let uniqueA = `How absurd to swallow a bird!\n`;
        let newStanzaA = newSent1 + uniqueA + newSent3A + sent2 + sent3 + sent4;
    ...
    for(i = 0; i <= animal.length - 1; i++){
       if(newSent3A.includes(animal[2])){
        let newSent1 = sent1.replace(animal[2], animal[3]);
        let sent3B = sent3A.replace(animal[2], animal[3]);
        let newSent3B = sent3B.slice(0, 25) + ` catch the ` + animal[2] + `.\n`; 
        let uniqueB = `Fancy that! She swallowed a cat!\n`;
        let newStanzaB = newSent1.concat(uniqueB + newSent3B + newSent3A + sent2 + sent3 + sent4);
 ...
 /*
    ...
    for(i = 0; i <= animal.length - 1; i++){
         if(newSent3B.includes(animal[3])){
            let newSent1 = sent1.replace(animal[3], animal[4]);
            let sent3C = sent3B.replace(animal[3], animal[4]);
            let newSent3C = sent3C.slice(0, 25) + ` catch the ` + animal[3] + `.\n`; 
            let uniqueC = `What a hog, to swallow a dog!\n`;
            let newStanzaC = newSent1.concat(uniqueC + newSent3C + newSent3B + newSent3A + sent2 + sent3 + sent4);
   ...
   for(i = 0; i <= animal.length - 1; i++){
         if(newSent3C.includes(animal[4])){
            let newSent1 = sent1.replace(animal[4], animal[5]);
            let sent3D = sent3C.replace(animal[4], animal[5]);
            let newSent3D = sent3D.slice(0, 26) + ` catch the ` + animal[4] + `.\n`; 
            let uniqueD = `She just opened her throat and swallowed a goat!\n`;
            let newStanzaD = newSent1.concat(uniqueD + newSent3D + newSent3C + newSent3B + newSent3A + sent2 + sent3 + sent4);
    ...
    for(i = 0; i <= animal.length - 1; i++){
         ...
         if(newSent3D.includes(animal[5])){
            let newSent1 = sent1.replace(animal[5], animal[6]);
            let sent3E = sent3D.replace(animal[5], animal[6]);
            let newSent3E = sent3E.slice(0, 25) + ` catch the ` + animal[5] + `.\n`; 
            let uniqueE = `I don't know how she swallowed a cow!\n`;
            let newStanzaE = newSent1.concat(uniqueE + newSent3E + newSent3D + newSent3C + newSent3B + newSent3A + sent2 + sent3 + sent4);
...
console.log(newStanzaA + newStanzaB + newStanzaC + newStanzaD + newStanzaE)
